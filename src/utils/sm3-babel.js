...
/**得到随机数**/
function getSignCf() {
  return new Date().getTime() + ':' + (Math.random() * 1000000000).toFixed(0);
}
...
function formDataFun(data) {
  ...
  if (data.indexOf("&") != -1) {
      ...
      theRequest[data.split('=')[0]] = data.split('=')[1];
  }
...
function setSign(json) {
  // console.log(json)
  // 格式化传递参数
  // json = formDataFun(json);
  // console.log(json)
  //  json = removeParam(json); // 移除前后台不兼容参数
  json = objKeySort(json); //a~z 排序
  json = JSON.stringify(json);
  // 转码参数
  json = decodeURIComponent(json);
  json = removeSymbol(json);
  // console.log(" ---------- >  " + json);
  return json;
}

/**
* 把 formData 参数 格式化成数组格式
* @param str
*/
